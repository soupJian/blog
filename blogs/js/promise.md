...
> 在面试中，手写 promise 也是热门考点\
> 手写 promise，能帮助我们更好的理解并运用 promise
...
似乎只运行了 resolve 但是没有触发 `.then()` 方法,因为在等一秒钟的时候，.then 方法已经运行结束了，但是 `#status` 此时还是 `pending` 状态，触发不了 `.then()`中的方法
...
从结果看，只执行了一次，这显然不符合要求，那么我们就应该把我们的 handler 设置一个数组，当调用一次`.then()` 时候，就读取一次，状态改变时候就删除一个
...
promise 只会 `resolve` 一次，但是可以 `.then()` 多次
