...
2. 利用 stripe 的 `createToken` 或者 `createPaymentMethod` 信息 获取到 信用卡的 vaultId
...
import { CHECKOUT_CREATE_MUTATION, CHECKOUT_COMPLETE_WITH_TOKENIZEDpYAMENTV3 } from "~/graphql/checkout";
...
const ShopifyPayments = ({ cartLines, vaultId }) => {
    ...
    const [checkoutCompleted] = useMutation(CHECKOUT_COMPLETE_WITH_TOKENIZEDpYAMENTV3);
    // 创建 checkout
